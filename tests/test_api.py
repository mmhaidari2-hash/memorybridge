...
def test_create_store_recall_update_flow_and_hashes_credentials():
    ...
    create_response = client.post("/v1/auth/token", json={"full_name": "Test User"})
...
def test_cross_user_session_access_is_blocked():
    ...
    user_a = client.post("/v1/auth/token", json={}).json()["user_token"]
    user_b = client.post("/v1/auth/token", json={}).json()["user_token"]
